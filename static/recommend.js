$(function() {
  ...
  $('.movie-button').on('click',function(){
    var my_api_key = '48dec75240074fd3c4d726b02b2c11d0';
    var title = $('.movie').val();
    if (title=="") {
...
function load_details(my_api_key,title){
  $.ajax({
    type: 'GET',
    url:'https://api.themoviedb.org/3/search/movie?api_key='+my_api_key+'&query='+title + "&language=pt-BR",
...
function get_movie_details(movie_id,my_api_key,arr,movie_title) {
  $.ajax({
    type:'GET',
    url:'https://api.themoviedb.org/3/movie/'+movie_id+'?api_key='+my_api_key + "&language=pt-BR",
...
function get_individual_cast(movie_cast,my_api_key) {
    ...
    for(var cast_id in movie_cast.cast_ids){
      $.ajax({
        type:'GET',
        url:'https://api.themoviedb.org/3/person/'+movie_cast.cast_ids[cast_id]+'?api_key='+my_api_key + "&language=pt-BR",
        async:false,
...
function get_movie_cast(movie_id,my_api_key){
    ...
    $.ajax({
      type:'GET',
      url:"https://api.themoviedb.org/3/movie/" + movie_id + "/credits?api_key=" + my_api_key + "&language=pt-BR",
      async:false,
